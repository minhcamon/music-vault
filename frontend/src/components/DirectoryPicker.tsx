...
import { Folder, FolderOpen, HardDrive, ArrowLeft, Check, Music } from 'lucide-react';
    ...
    <div className="space-y-4">
          ...
          browseResult?.items.map((item) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                {item.audioCount > 0 && (
                  <span className="mono-tech text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1">
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded bg-accent-primary/20 hover:bg-accent-primary text-accent-primary hover:text-white text-[11px] font-medium border border-accent-primary/30 transition-all opacity-0 group-hover:opacity-100"
                >
                  Chọn folder này
                </button>
              </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/10">
        <span className="text-[11px] text-text-secondary truncate max-w-[280px]">
          Đang ở: <strong className="text-text-primary font-mono">{currentPath}</strong>
        </span>
        <div className="flex gap-2">
          ...
          >
            ...
            <span>Chọn thư mục hiện tại</span>
          </button>
        </div>
      </div>
    </div>
