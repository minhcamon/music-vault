...
export const Sidebar: React.FC = () => {
  ...
  const navItems: { mode: ViewMode; label: string; icon: React.ReactNode; count: number }[] = [
    ...
    { mode: 'sources', label: 'Nguồn nhạc (Cloud/Local)', icon: <HardDrive className="w-5 h-5" />, count: sources.length },
  ];
  ...
  return (
    <aside className="w-64 border-r border-vault-border bg-vault-bg/80 backdrop-blur-xl flex flex-col justify-between p-4 select-none">
      ...
      {/* Footer Info */}
      <div className="p-3 rounded-xl bg-white/5 border border-vault-border text-xs text-vault-muted space-y-1">
        <div className="font-semibold text-vault-text">Pure Client-Side</div>
        <div>No-Backend • No-Server DB</div>
        <div className="text-[10px] text-vault-accent/80 font-mono">Dexie IndexedDB Engine</div>
      </div>
    </aside>
  );
};
